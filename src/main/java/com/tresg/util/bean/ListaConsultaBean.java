...
public class ListaConsultaBean implements Serializable {
	...
	public List<VentaJPA> listarVentaPorCliente(String cliente) {
		...
		sVenta.listaVenta().stream()
				.filter(v -> v.getCliente().getNombre().toLowerCase().contains(cliente.toLowerCase()))
				.sorted(Comparator.comparing(VentaJPA::getNumComprobante).reversed()).forEach(ventas::add);
		return ventas;
	}
	...
	public double acumuladoContadoVentaXFecha(Date fecha) {
		return sVenta.listaVenta().stream()
				.filter(f -> (fecha.equals(f.getFecha()) && f.getPago().getCodPago() == 1))
				.mapToDouble(m -> m.getMonto().doubleValue()).sum();
	}

	public double acumuladoDepositoVentaXFecha(Date fecha) {
		return sVenta.listaVenta().stream()
				.filter(f -> (fecha.equals(f.getFecha()) && f.getPago().getCodPago() == 3))
				.mapToDouble(m -> m.getMonto().doubleValue()).sum();
	}

	public double acumuladoCreditoVentaXFecha(Date fecha) {
		return sVenta.listaVenta().stream()
				.filter(f -> (fecha.equals(f.getFecha()) && f.getPago().getCodPago() == 2))
				.mapToDouble(m -> m.getMonto().doubleValue()).sum();
	}
